...
export async function detectEdges(
  ...
  const cleaned = removeSmallEdges(edges, canvas.width, canvas.height, threshold)
...
export function applyWatershed(edgeData: EdgeData): string {
  const segmented = watershed(edgeData.cleaned, edgeData.width, edgeData.height)
  return segmentedToDataURL(segmented, edgeData.width, edgeData.height)
}
...
function segmentedToDataURL(regions: Int32Array, width: number, height: number): string {
  ...
  for (let i = 0; i < regions.length; i++) {
    ...
    const color = region === -1 ? [0, 0, 0] : colors[region]
